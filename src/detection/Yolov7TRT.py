...
class YoLov7TRT(object):
    ...
    def post_process(self, output, origin_h, origin_w):
        """
        description: postprocess the prediction to only return detections for class 0
        param:
            output:     A numpy like [num_boxes, cx, cy, w, h, conf, cls_id, cx, cy, w, h, conf, cls_id, ...]
            origin_h:   height of original image
            origin_w:   width of original image
        return:
        ...
        # Filter predictions to only include class 0 before NMS
        class_0_pred = pred[pred[:, 5] == 0]
        # Do nms if there are any predictions for class 0
        if class_0_pred.shape[0] > 0:
            ...
            boxes = np.array([])  # No detections found for class 0
